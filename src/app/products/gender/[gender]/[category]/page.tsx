// src/app/products/gender/[gender]/[category]/page.tsx
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        // Look up the category by slug directly from DB — no string manipulation
        ...
        if (!cat) {
          ...
          router.push(`/products/gender/${genderParam}`);
          return;
        }

        setCategory(cat as CategoryObj);

        // Fetch all products and filter by gender + category_id
        const products = await getProducts();
        const productsWithSlugs = products.map(addSlugToProduct);

        const filtered = productsWithSlugs.filter(
          (p) => p.gender === genderValue && p.category_id === cat.id
        );
